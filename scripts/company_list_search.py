...
"""
跨境魔方公司列表搜索
根据海关贸易数据搜索采购商或供应商公司，使用游标查询支持大数据量检索。
"""
import argparse
import sys
from common import (
...
def search_company_list(params: dict, cursor: str = None) -> dict:
    ...
    # 发起API请求
    response = make_request('/agent/customs/company/list', request_params)
    return response


def main():
    parser = argparse.ArgumentParser(
        description='从跨境魔方开放平台搜索采购商/供应商公司'
    )
    parser.add_argument(
        '--params',
        help='搜索参数JSON字符串（必须包含companyType）'
    )
    parser.add_argument(
        '--task_id',
    ...
    # 验证 companyType 值
    if 'companyType' not in params or params['companyType'] not in [1, 2]:
        print("错误：companyType 必须是 1（供应商）或 2（采购商）", file=sys.stderr)
        sys.exit(1)
